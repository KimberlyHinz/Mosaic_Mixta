...
The genomes were annotated with Prokka v1.14.1 [@Seemann2014] and homologous genes were extracted using GET_HOMOLOGUES software package [@Contreras-Moreira2013] with the bidirectional best-hit search algorithm using default parameters.

Within `R`, a filtering function was created to pass forward fasta files with ten, non-truncated sequences. The minimum gene length to pass through the filter was 90% the length of the longest sequence in that file.
...
The software package GET_HOMOLOGUES extracted 1284 genes that were common between the ten species. Of those genes, 1035 passed a filtering process that allowed through only files with 10 sequences and no truncated sequences.
